...
public class YouTubeChannel implements Serializable {
	...
	private void parse(Channel channel) {
		...
		if (branding != null)
			this.bannerUrl = GoTubeApp.isTablet() ? branding.getImage().getBannerTabletHdImageUrl() : branding.getImage().getBannerMobileHdImageUrl();

		ChannelStatistics statistics = channel.getStatistics();
		if (statistics != null) {
			this.totalSubscribers = String.format(GoTubeApp.getStr(R.string.total_subscribers),
